#This codefile runs the testing of the best model (selected after Validation_multi.py) from one CNN architecture that is defined in the beginning on one sample and all genes.
#This code requires the best model's saved model parameters from Train_multi.py, the loaded data of one sample from the Dataloader_Test_multi.py and the corresponding H&E WSIs.
...
from Dataloader_Test_multi import *
import pickle
...
#------------------------------------------------------------------------------
pretrained_model = "resnet50" #adapt to the model architecture that was the best during validation
load_metric = "correlation" #adapt to loss if needed
#------------------------------------------------------------------------------


#Set batchsize and learning rate
#Adapt to the best performing parameters during validation
#------------------------------------------------------------------------------
batch_size = 64 #This was the best performing batch size for our samples during validation
lr = "LLR" #This was the best performing learning rate for our samples during validation
#------------------------------------------------------------------------------


#Creating test log text file
#------------------------------------------------------------------------------
test_log = data_dir+"XXX.txt" #adapt to desired output name
...
#------------------------------------------------------------------------------
def testing(gene_list, test_loader, best_model):
  best_model.eval()
  ...
  with torch.no_grad():
      for images, gene1_values, gene2_values, gene3_values, gene4_values, path in test_loader:
          ...
          temp_df.insert(0, test_columns[c], gene1_values[:,0].cpu())
          c+=1
          temp_df.insert(0, test_columns[c], g1_output[:,0].cpu())
          c+=1
          temp_df.insert(0, test_columns[c], gene2_values[:,0].cpu())
          c+=1
          temp_df.insert(0, test_columns[c], g2_output[:,0].cpu())
          c+=1
          temp_df.insert(0, test_columns[c], gene3_values[:,0].cpu())
          c+=1
          temp_df.insert(0, test_columns[c], g3_output[:,0].cpu())
          c+=1
          temp_df.insert(0, test_columns[c], gene4_values[:,0].cpu())
          c+=1
          temp_df.insert(0, test_columns[c], g4_output[:,0].cpu())
...
#------------------------------------------------------------------------------
def pearson_calc(test_df, true, preds, pearson_dict, batch_size, lr, sample):
  for j in range(len(true)):
    pearson_name = true[j].replace("true", "pearson")
    pearson_name = pearson_name + "_"+str(batch_size)+"_"+lr+"_"+sample
...
#Create spatial plots of test samples (ground truth and prediction)
#------------------------------------------------------------------------------
def spatial_plot(test_df, true, preds, batch_size, lr, sample): 
#Load spatial matrix
...
#Save predicted AnnData object
  adata_test.write(data_dir + "XXX"+sample+".h5ad") #adapt to the desired output name and directory

#Plot spatial gene truth and prediction
  for l in range(len(true)):
    spatial_plot_name = data_dir+"/XXX/"+sample+".tif" #adapt to desired output name and directory
...
#Running the whole testing process
#------------------------------------------------------------------------------
pearson_dict = {}
...
load_data_dir = 'XXX' #include the directory of the .pt file with best performing model parameters created from Train_multi.py and selected in Validation_multi.py
...
test_df = testing(gene_list, test_loader, best_model)
...
test_df_save = data_dir + "XXX"+sample+".csv" #adapt to your desired output name and directory
...
#The outputs of this codefile are: .csv file with ground truth and predicted values for all genes on one test sample, the corresponding spatial plots for ground truth and prediction,
#the AnnData object of ground truth and prediction, a .csv file of Pearson's r correlation for all genes on one test sample, 
#a test_log .txt file with model parameters and Pearson's r correlation.
